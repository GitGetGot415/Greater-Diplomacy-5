...
from ui.list_select_screen import MultiSelectScreen
from ui.player_diplomacy_menus import _run_pygame_sub_screen, build_choice_button
...
class Diplomacy_Edit_Screen(MapOverlayScreen):
    ...
    def __init__(self, map_screen, target, active_countries):
        super().__init__(map_screen, pygame.Rect(0, 0, 560, 460))
        self.panel_rect.center = (c.SCREEN_WIDTH // 2, c.SCREEN_HEIGHT // 2)
        self.back_state = "MAP"
        self.target = target
        ...
        self.at_war_with = set(data.get("at_war_with", []))
        self.faction_name_field = TextField(0, 0, 260, 36, data.get("faction", ""))
        self.is_leader = data.get("is_faction_leader", False)

        faction = data.get("faction", "")
        self.faction_members = set(
            other for other in active_countries
            if other != target and faction and map_screen.nation_data.get(other, {}).get("faction", "") == faction
        )

        master_val = data.get("master", "")
        self.master = master_val if master_val else "None"
        self.puppet_type = data.get("puppet_type", c.PUPPET_TYPE_AUTONOMOUS) or c.PUPPET_TYPE_AUTONOMOUS

        self.refresh_ui()

    def refresh_ui(self):
        p = self.panel_rect
        self.faction_name_field.rect.topleft = (p.x + 220, p.y + 245)

        self.elements = [
            Button(50, c.TOP_BAR_UI_CENTER_Y, "small", "red", "Back", self.exit_screen),
            Button(p.centerx - 100, p.bottom - 55, "medium", "green", "Save Changes", self.save),

            Button(p.x + 30, p.y + 90, "medium", "orange", f"At War With ({len(self.at_war_with)})", self.edit_war_list),
            build_choice_button(p.x + 300, p.y + 90, "medium",
                                "Faction Leader" if self.is_leader else "Not Leader",
                                True, self.is_leader, self.toggle_leader),

            Button(p.x + 30, p.y + 300, "medium", "orange", f"Faction Members ({len(self.faction_members)})",
                  self.edit_faction_members),

            Button(p.x + 30, p.y + 360, "medium", "blue", f"Master: {self.master}", self.edit_master),
            Button(p.x + 300, p.y + 360, "medium", "blue", f"Puppet Type: {self.puppet_type}", self.cycle_puppet_type),

            self.faction_name_field,
        ]
    ...
    def cycle_puppet_type(self):
        self.puppet_type = (c.PUPPET_TYPE_INTEGRATED if self.puppet_type == c.PUPPET_TYPE_AUTONOMOUS
                            else c.PUPPET_TYPE_AUTONOMOUS)
        self.refresh_ui()

    def _others(self):
        return [name for name in self.active_countries if name != self.target]

    def edit_war_list(self):
        def on_confirm(selected):
            self.at_war_with = selected
            self.refresh_ui()
        screen = MultiSelectScreen(self.map_screen, "At War With", f"Nations {self.target} is at war with:",
                                   self._others(), self.at_war_with, on_confirm)
        _run_pygame_sub_screen(self.map_screen, screen)
        self.refresh_ui()

    def edit_faction_members(self):
        def on_confirm(selected):
            self.faction_members = selected
            self.refresh_ui()
        screen = MultiSelectScreen(self.map_screen, "Faction Members", f"{self.target}'s faction members:",
                                   self._others(), self.faction_members, on_confirm)
        _run_pygame_sub_screen(self.map_screen, screen)
        self.refresh_ui()

    def edit_master(self):
        options = ["None"] + self._others()

        def on_pick(val):
            self.master = val
            self.refresh_ui()
        queries.open_listbox_selector(self.map_screen, "Master Nation", "Select master nation:", options, on_pick)
        self.refresh_ui()
    ...
    def save(self):
        ...
        # 1. Wars (bidirectional)
        for c_name in self.active_countries:
            if target in nation_data[c_name].get("at_war_with", []):
                nation_data[c_name]["at_war_with"].remove(target)

        data["at_war_with"] = list(self.at_war_with)
        ...
        # 2. Factions
        new_faction = self.faction_name_field.text.strip()
        ...
        for c_name in self.active_countries:
            if c_name == target:
                continue
            if c_name in self.faction_members:
                nation_data[c_name]["faction"] = new_faction
                if new_faction:
                    nation_data[c_name]["is_faction_leader"] = False
            elif nation_data[c_name].get("faction", "") == new_faction and new_faction != "":
                nation_data[c_name]["faction"] = ""
                nation_data[c_name]["is_faction_leader"] = False
    ...
    def draw_content(self, surface):
        p = self.panel_rect
        ui_bars.draw_modal_box(surface, p, bg_color=(30, 30, 45), border_color=(255, 50, 50), border_width=2)
        ui_bars.draw_centered_title(surface, f"Editing: {self.target}", p.y + 20, "heading2")

        label = fonts.get("normal").render("Faction Name:", True, (220, 220, 220))
        surface.blit(label, label.get_rect(midright=(p.x + 210, p.y + 263)))

def open_diplomacy_editor_screen(map_screen):
    ...
    def on_pick(target):
        screen = Diplomacy_Edit_Screen(map_screen, target, active_countries)
        _run_pygame_sub_screen(map_screen, screen)

    queries.open_listbox_selector(map_screen, "Global Diplomacy & Factions Editor", "Select a nation to edit:",
                                  active_countries, on_pick)
