...
class MidpointReinforcementTests(unittest.TestCase):
    def test_ai_orders_a_unit_into_an_active_midpoint_battle(self):
        screen = BombardmentScreen()
        home = screen.map_data["home"]
        near = screen.map_data["near"]

        engaged = {"type": "Infantry Type 1936", "owner": "Avaria",
                   "health": 1000, "attack": 100, "defense": 50,
                   "_edge_battle": {"origin_id": 1, "destination_id": 2}}
        enemy_engaged = {"type": "Infantry Type 1936", "owner": "Bruland",
                         "health": 1000, "attack": 100, "defense": 50,
                         "_edge_battle": {"origin_id": 2, "destination_id": 1}}
        reinforcement = {"type": "Infantry Type 1936", "owner": "Avaria",
                         "health": 1000, "attack": 100, "defense": 50,
                         "order": {"type": "MOVE", "path": []}}
        home["units"] = [engaged, reinforcement]
        near["units"] = [enemy_engaged]

        blocked, reinforce, edge_targets = ai_movement._edge_reinforcement_options(
            screen, "Avaria")
        ctx = empty_ctx(
            target_destinations=[2],
            target_assignments={2: 0},
            target_stacks={2: 0},
            target_capacity={},
            blocked_edge_pairs=blocked,
            reinforce_edge_pairs=reinforce,
            edge_reinforcement_targets=edge_targets,
        )

        ai_movement._assign_unit_orders(
            screen, "Avaria", [(reinforcement, home)], ctx,
            {1, 2, 3}, set(), {1: [2], 2: [1, 3], 3: [2]})

        self.assertEqual(reinforcement["order"]["path"], [2])

    def test_ai_orders_the_reverse_endpoint_into_the_same_midpoint_battle(self):
        screen = BombardmentScreen()
        home = screen.map_data["home"]
        near = screen.map_data["near"]
        ...
        enemy_engaged = {"type": "Infantry Type 1936", "owner": "Bruland",
                         "health": 1000, "attack": 100, "defense": 50,
                         "_edge_battle": {"origin_id": 2, "destination_id": 1}}
        reinforcement = {"type": "Infantry Type 1936", "owner": "Bruland",
                         "health": 1000, "attack": 100, "defense": 50,
                         "order": {"type": "MOVE", "path": []}}
        home["units"] = [engaged]
        near["units"] = [enemy_engaged, reinforcement]

        blocked, reinforce, edge_targets = ai_movement._edge_reinforcement_options(
            screen, "Bruland")
        ctx = empty_ctx(
            target_destinations=[1],
            target_assignments={1: 0},
            target_stacks={1: 0},
            target_capacity={},
            blocked_edge_pairs=blocked,
            reinforce_edge_pairs=reinforce,
        ...
        ai_movement._assign_unit_orders(
            screen, "Bruland", [(reinforcement, near)], ctx,
            {1, 2, 3}, set(), {1: [2], 2: [1, 3], 3: [2]})

        self.assertEqual(reinforcement["order"]["path"], [1])
