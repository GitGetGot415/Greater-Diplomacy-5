...
async def resolve_turn_logic(map_screen): # Renamed from resolve_turn
    ...
    # Defense-area returns are deliberately queued only after every submitted
    ...
    queued_defense_returns = queries.queue_idle_army_defense_orders(map_screen)
    if queued_defense_returns:
        print(f"[SYSTEM] Queued {queued_defense_returns} army defense return order(s).")
    await asyncio.sleep(0)
