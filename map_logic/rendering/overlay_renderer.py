        ...
        for offset in offsets:
            ...
            if (-CULL_MARGIN < sx < surface.get_width() + CULL_MARGIN
                ...
                if map_screen.secondary_mode == "UNITS":
                    ...
                    units_by_alpha = {}
                    target_area_picker_alpha = getattr(
                        map_screen, "_target_area_unit_box_alpha", 255)
                    for unit in display_units:
                        alpha = min(strategic_unit_alphas.get(id(unit), 255),
                                    target_area_picker_alpha)
                        if alpha > 0:
...
def draw_unit_icon(map_screen, surface, sx, sy, province, is_partial=False,
    ...
    # Target-area pickers make force boxes translucent so their tiles remain
