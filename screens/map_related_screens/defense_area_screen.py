"""Army-group target-area map picker."""
...
class DefenseAreaScreen(MapOverlayScreen):
    """Select tiles that an army should cover or return to when it is idle."""

    overlay_alpha = 0
    CENTER_PANEL = False
    PANEL_TITLE = "Create Target Area"
    ...
    def confirm(self):
        if not self.map_screen.can_select_map_units():
            self.map_screen.show_feedback(
                "Turn submitted or unavailable; unsubmit to edit target areas.")
            return
        ...
        self.map_screen.show_feedback(
            f"Target area saved: {len(army['defense_area'])} {tile_word}; "
    ...
    def draw(self, surface):
        ...
        previous_alpha = getattr(self.map_screen, "_target_area_unit_box_alpha", None)
        self.map_screen._target_area_unit_box_alpha = self.UNIT_BOX_ALPHA
        try:
            super().draw(surface)
        finally:
            if previous_alpha is None:
                del self.map_screen._target_area_unit_box_alpha
            else:
                self.map_screen._target_area_unit_box_alpha = previous_alpha
    ...
    def draw_content(self, surface):
        ...
        instructions = (
            f"Select target tiles for {army_name}. Confirming gives its units orders now."
        )
